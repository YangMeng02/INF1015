...
Designer* chercherDesigner(ListeJeux liste, Designer* ptrDesigner)
{
	for (int iterJeu = 0; iterJeu < liste.nElements; iterJeu++)
	{
		Jeu jeu = *liste.elements[iterJeu];
		for (int iterDesigner = 0; iterDesigner < jeu.designers.nElements; iterDesigner++)
		{
			if (jeu.designers.elements[iterDesigner] == ptrDesigner)
			{
				return jeu.designers.elements[iterDesigner];
			}
		}
	}
	return 0;
...
void changerTailleListeJeu(ListeJeux liste, int newCapacite)
{
	liste.capacite = newCapacite;
	Jeu** newList = new Jeu*[newCapacite];
	for (int element = 0; element < liste.nElements; element++)
	{
		newList[element] = liste.elements[element];
	}
	liste.elements = {};
	liste.elements = newList;
	delete[] newList;
}
...
void ajouterJeu(ListeJeux liste, Jeu* ptrJeu)
{
	if (liste.nElements == 0)
	{
		Jeu** newList = new Jeu*[1];
		liste.elements = newList;
		delete[] newList;
		liste.nElements = 0;
		liste.capacite = 1;
	...
	{
		changerTailleListeJeu(liste, liste.capacite * 2);
	}
	liste.elements[liste.nElements] = ptrJeu;
	liste.nElements++;
}
...
void enleverJeu(ListeJeux liste, Jeu* ptrJeu)
{
	for (int element = 0; element < liste.nElements; element++)
	{
		if (liste.elements[element] == ptrJeu)
		{
			liste.nElements--;
			liste.elements[element] = liste.elements[liste.nElements];
...
ListeJeux creerListeJeux(const string& nomFichier)
{
	...
	for([[maybe_unused]] int n : iter::range(nElements))
	{
		...
		ajouterJeu(listeJeux, lireJeu(fichier)); //TODO: Ajouter le jeu à la ListeJeux.
	}
